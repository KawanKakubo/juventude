...
<div class="bg-white rounded-lg shadow-md p-6">
    <form action="{{ route('admin.posts.update', $post) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-6">
            <label class="block text-gray-700 text-sm font-bold mb-2">Galeria Atual</label>
                    ...
                    @foreach($post->photos as $photo)
                        <div class="relative group">
                            ...
                            <div class="absolute top-2 right-2">
                                <form action="{{ route('admin.photos.destroy', $photo->id) }}" method="POST" onsubmit="return confirm('Tem certeza que deseja excluir esta foto?')">
                                    @csrf
                                    @method('DELETE')
            ...
            @else
                <p class="text-gray-500 text-sm">Nenhuma foto na galeria ainda.</p>
            @endif
        </div>

        <div class="mb-6">
            <label for="photos" class="block text-gray-700 text-sm font-bold mb-2">Adicionar Novas Fotos à Galeria</label>
        ...
        <div class="mb-6">
            <label class="flex items-center">
                ...
                <span class="ml-2 text-gray-700">Publicar esta notícia</span>
            </label>
        </div>

        <div class="flex gap-4">
            <button type="submit" class="bg-[#1e3a5f] hover:bg-[#2d5a8f] text-white font-bold py-3 px-6 rounded-lg transition-all">
                Atualizar Notícia
            </button>
            <a href="{{ route('admin.posts.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white font-bold py-3 px-6 rounded-lg inline-block text-center">
                Cancelar
            </a>
        </div>
    </form>
...
@push('scripts')
<script>
    console.log('Script de edição carregado!');
